...
class AddItineraryScreen extends StatefulWidget {
  ...
  const AddItineraryScreen({super.key, required this.plan});
...
class _AddItineraryScreenState extends State<AddItineraryScreen> {
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (widget.plan.startDate != null) {
                final date = widget.plan.startDate!.add(Duration(days: index));
                dayLabel = "D$day (${DateFormat('dd/MM').format(date)})";
              }

              return Padding(
                padding: const EdgeInsets.only(right: 10),
                child: ChoiceChip(
                  label: Text(dayLabel, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
                  selected: isSelected,
  ...
  void _onSave() {
    ...
    context.read<AppState>().addItineraryItem(
          widget.plan.id,
          PlanItineraryItem(
            activity: activity,
            day: selectedDay,
